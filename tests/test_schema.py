...
class TestBackwardCompatibility:
    """Tests ensuring backward compatibility with existing data_schema.py."""

    def test_existing_judgment_sample_still_works(self):
        """Original JudgmentSample from data_schema.py should still work."""
        from src.data_schema import JudgmentSample

        sample = JudgmentSample(
            proposition="Test proposition",
            evidence="Test evidence",
            conditions=["Condition 1"],
            conditions_fulfilled={"Condition 1": True},
            judgment="Yes",
            judgment_reasoning="Conditions met.",
            is_virtually_unconditioned=True
        )

        assert sample.judgment == "Yes"

    def test_enhanced_sample_extends_original(self):
        """EnhancedJudgmentSample should be usable where JudgmentSample is expected.

        Note: This tests conceptual compatibility, not strict inheritance.
        """
        from src.schema.extended_schema import EnhancedJudgmentSample

        # Enhanced sample should have all core judgment sample fields
        sample = EnhancedJudgmentSample(
            proposition="Test",
            domain="empirical",
